...
pub trait BiomeGeneration {
    fn generate(&self, seed: u32, col: ColPos, params: &BiomeParameters) -> Vec<Layer>;
}

impl BiomeGeneration for Biome {
    fn generate(&self, seed: u32, col: ColPos, params: &BiomeParameters) -> Vec<Layer> {
        match self {
            Biome::PolarOcean => generate_polar_ocean(seed, col, params),
            Biome::Ocean => generate_ocean(seed, col, params),
            Biome::Mountain => generate_mountain(seed, col, params),
            Biome::Desert => generate_desert(seed, col, params),
            Biome::Jungle => generate_jungle(seed, col, params),
            Biome::Canyon => generate_canyon(seed, col, params),
            Biome::Tundra => generate_tundra(seed, col, params),
            // Plain is the default for any not yet implemented biomes
            _ => generate_plain(seed, col, params),
        }
    }
}
